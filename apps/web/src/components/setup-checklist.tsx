...
export function getSetupFlags(botId: string) {
  return {
    ...
    previewOpened:
      localStorage.getItem(`ragify-preview-opened-${botId}`) === "1",
    settingsVisited:
      localStorage.getItem(`ragify-settings-visited-${botId}`) === "1",
  };
}

export function markSetupFlag(
  botId: string,
  flag: "embedCopied" | "previewOpened" | "settingsVisited"
) {
  const keys = {
...
export function SetupChecklist({
  ...
  const allCoreDone =
    hasSource && hasReady && flags.embedCopied && flags.previewOpened;

  const steps: Step[] = useMemo(
    () => [
      {
        id: "context",
        label: "Add context (text, URL, or PDF)",
        done: hasSource,
      },
      { id: "index", label: "Wait for indexing to finish", done: hasReady },
      {
        id: "customize",
        label: "Customize welcome message & color",
        done: flags.settingsVisited,
        optional: true,
      },
      ...
      <ul className="space-y-1 mb-4">
        {steps.map((step) => (
          <li key={step.id} className="setup-step">
            <span
              className={`setup-step-icon ${step.done ? "done" : "pending"}`}
            >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {allCoreDone && (
          <>
            ...
            <Link
              to={`/dashboard/bots/${botId}?tab=usage`}
